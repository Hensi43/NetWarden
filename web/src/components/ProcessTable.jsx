    ...
    return (
        <div className="flex flex-col h-full bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between bg-slate-50">
                <h3 className="font-semibold text-slate-900 text-sm">Active Processes</h3>
                ...
                <div className="flex items-center gap-2">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Filter..."
                            className="h-8 pl-8 pr-3 text-xs bg-white border border-slate-300 rounded-md focus:outline-none focus:border-indigo-500 text-slate-900 w-40 transition-colors"
                            value={filter}
                            onChange={(e) => setFilter(e.target.value)}
                        />
                    </div>
                    <button className="h-8 w-8 flex items-center justify-center rounded-md border border-slate-300 bg-white hover:bg-slate-50 text-slate-600">
            ...
            <div className="flex-1 overflow-auto">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 sticky top-0 z-10 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                        <tr>
                            <th className="px-4 py-3 border-b border-slate-200">Process Info</th>
                            <th className="px-4 py-3 border-b border-slate-200 text-right">Download</th>
                            <th className="px-4 py-3 border-b border-slate-200 text-right">Upload</th>
                            <th className="px-4 py-3 border-b border-slate-200 text-center">Priority</th>
            ...
            {/* Footer */}
            <div className="px-4 py-2 border-t border-slate-200 bg-slate-50 text-xs text-slate-500 flex justify-between">
...
const ProcessRow = ({ proc }) => {
    return (
        <tr className={clsx(
            "group hover:bg-slate-50 transition-colors text-sm",
            ...
            <td className="px-4 py-2.5 text-right">
                <button className="text-xs px-2 py-1 rounded border border-slate-300 hover:bg-slate-100 text-slate-600">
                    Details
                </button>
            </td>
        </tr>
